...
"Jangan kirim link Grub",
"Jangan kirim link Http",
...
let row = Object.keys(namop, desop, idop).map((v, index) => ({
	...
	let button = {
		...
		description: `Hai ${name}, Silakan pilih ${command} Options di tombol di bawah...\n\n\nKetik ulang perintah *${usedPrefix + command}* teks anda untuk mengubah pilihan isi list nya`,
		footerText: wm
	}
  ...
  switch (type) {
    ...
    case 'statusonly':
      ...
      break
    default:
      if (!/[01]/.test(command)) return conn.sendListM(m.chat, button, row, fdocs)
      throw false
  }
  
  
  conn.send2Button(m.chat, `*${htki} OPTIONS ${htka}*
🗂️ *Type:* ${type} 
📊 *Status:* Succes ✅
